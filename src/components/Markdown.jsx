...
window.addEventListener('load',init_emoji);
    ...
    case ':':
      ...
      useEffect(() => {
        async function checkEmoji() {
          const temp = await Verif_emoji(part[1][0]);
          setEmoji(temp);
        }
        checkEmoji();
      }, [part]);
      ...
      if (emoji) {
        return (
          <img src={"emoji/" + emoji} alt={emoji} style={{maxHeight : 20, maxWidth : 40}} />
        );
      } else {
        return (
          <span>:<MarkdownParagraph pArray={part[1]} />:</span>
        );
      }
    default:
      return type;
...
async function Verif_emoji(name) {
  //const a = await fetch('/emoji').then((response) => response.json())
  const a = emoji

  for (let i = 0; i < a.length; i++) {
    if (a[i].replace(/\.[^\.]+$/, "") == name.toLowerCase()) {
      return a[i];
    };
  }
  return false;
