...
export function addNews(post) {
  if (!post) {
    throw new Error(`Can't post an empty article`);
  }

  const fileName = '../src/News.json';
  const dir = "../src/"
  var file;
  var filelength;
  ...
  try {
    ...
    throw new Error('Couldn\'t acces News file ' + fs.readdirSync(dir));
  }

  try {
    filelength = Object.keys(file).length + 1;
  } catch {
    throw new Error('Couldn\'t acces News obj');
  }
  ...
  try {
    ...
    throw new Error('Couldn\'t write News file');
  }

  return `#${filelength} News article was posted`;
}
